...
function columnTogglerClearAllButton(){
	for (let i = 0; i < $('.ui-columntoggler-items').length; i++) {
		let columnTogglerElement = $('.ui-columntoggler-items')[i];
		for (let j = 0; j < columnTogglerElement.getElementsByTagName('li').length; j++) {
			let liElement = columnTogglerElement.getElementsByTagName('li')[j];
			let columnTogglerTagName = liElement.getElementsByTagName('label')[0].innerHTML;
			if(!columnTogglerTagName || columnTogglerTagName.length === 0){
				$(liElement).remove();
			}
		}
	}
