...
public class KullaniciIletisimServiceImpl extends BaseCrudServiceImpl<KullaniciIletisimDto, KullaniciIletisim> implements KullaniciIletisimService {
	...
	public QueryResult<KullaniciIletisimDto> listele(int firstRecord, int pageSize, Map<String, SortMeta> sort, KullaniciIletisimDto sorguKriteri, String sorgu) {
		int pageNumber = firstRecord / pageSize;

		Page<KullaniciIletisim> page = listeleInternal(pageSize, pageNumber, sort, sorguKriteri, sorgu);
		return new QueryResult<KullaniciIletisimDto>(dtoMapper.entityListToDtoList(page.getContent()), (int) page.getTotalElements());
	}

	private Page<KullaniciIletisim> listeleInternal(int pageSize, int pageNumber, Map<String, SortMeta> sort, KullaniciIletisimDto sorguKriteri, String sorgu) {
		return customKullaniciIletisimRepository.sorgula(pageSize, pageNumber, sort, dtoMapper.dtoToEntity(sorguKriteri), sorgu);
	}
