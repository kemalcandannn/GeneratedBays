...
public abstract class BaseCrudController<DTO extends BaseDto> {
	...
	private DTO secilen;
    private List<DTO> secilenListesi;
	...
	public void formTemizle() {
		...
		secilenListesi = null;
		list.setSorgu(null);
		setSorguKriteri(createNewModel());
	}
	...
	public void onCellEdit(CellEditEvent<DTO> event) {
		setSecilen(list.getDatasource().get(event.getRowIndex()));
		kaydet(null);
	}
	...
	public void setSecilenListesi(List<DTO> secilenListesi) {
		this.secilenListesi = secilenListesi;
	}

	public List<DTO> getSecilenListesi() {
		return secilenListesi;
	}
	...
	public void guncelleFormAc() {
		if (secilenListesi == null || secilenListesi.size() == 0) {
			addError(BaysResource.get("kayit_secilmedi"));
			return;
		}
		
		secilen = secilenListesi.get(0);
	}
	...
	public DTO kaydet(String dialogName) {
	...
	public boolean sil() {
		...
		if (validateSil()) {
			preSil();
			ret = nesneSil(secilenListesi);
			if (ret == false)
				return ret;
			if (LOGGER.isDebugEnabled())
				LOGGER.debug(BaysResource.get("kayit_silindi") + ": " + secilenListesi.toString());
	...
	public boolean nesneSil(List<DTO> entityList) {
		try {
			crudService.sil(entityList);
